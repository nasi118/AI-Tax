/* ============================================================================
   /api/grok — secure server-side proxy to the xAI Grok API.

   The XAI_API_KEY lives only in the deployment environment (Vercel project
   settings); it is never present in browser code or in this repository.

   Controls:
     - POST only
     - request-size ceiling
     - per-IP rate limit (best effort per warm serverless instance)
     - conversation history sanitized and truncated
     - upstream timeout
     - controlled error messages (no upstream internals leaked)
     - usage logged without logging tax data
   ========================================================================== */

const MAX_BODY_BYTES = 200 * 1024;
const MAX_MESSAGES = 24;
const MAX_MESSAGE_CHARS = 24 * 1024;
const MAX_SYSTEM_CHARS = 48 * 1024;
...
module.exports = async function handler(req, res) {
  ...
  if (!apiKey) {
    res.status(501).json({ error: "AI review is not configured on this deployment (XAI_API_KEY is not set)." });
    return;
  }
  ...
  // Sanitize: only user/assistant roles, plain strings, bounded count/length.
  ...
  try {
    ...
    console.log(JSON.stringify({ evt: "grok_proxy", ip, model, ms: Date.now() - started, error: timedOut ? "timeout" : "network" }));
    res.status(504).json({ error: timedOut ? "The AI service timed out — try a shorter question." : "The AI service is unreachable." });
    return;
  }

  if (!upstream.ok) {
    console.log(JSON.stringify({ evt: "grok_proxy", ip, model, ms: Date.now() - started, upstreamStatus: upstream.status }));
  ...
  // Usage is logged for capacity monitoring; message content never is.
