/* ==== ARCHIVED — 08a-scenarios-ledger ==== */
/* ============================================================================
   ARCHIVED: THE SCENARIOS LEDGER (retained, not deleted, NOT LOADED)

   This file holds the line-by-line scenario comparison ledger that used to be
   the Scenarios tab, together with every editor, drill-down and modal that
   belonged to it. It was lifted verbatim out of src/08-pages.js when the
   Scenarios tab was given over to the 1040 Planner module
   (see src/27-scenarios-planner.js).

   IT IS DEPARTED FROM THE RUNNING APP:
     · index.html does not load it.
     · Nothing routes to ScenariosPage any more.
     · It therefore takes part in no calculation — the Scenarios tab is
       unlinked from the workbench engine, which is the point of the change.

   It is kept because the ledger encodes a great deal of reviewed work: the
   Form 1040 line map, the Schedule C / passthrough / S-corp / Schedule 1 /
   Schedule A editors, the strategy library, and the drill-down wiring. If any
   of it is wanted again, it is here, unmodified, ready to be re-linked.

   To bring it back, load this file from index.html AFTER src/08-pages.js and
   restore the `tab === "scenarios"` mount in src/10-app.js. Note that it
   depends on helpers defined across 00-format, 05-ui-primitives, 07-analysis,
   19-ui-shell and 21-calculators, so its load order matters.

   Archived: 2026-08-30. Verbatim from src/08-pages.js lines 503-2831.
   ========================================================================== */
...
/* ============================================================================
   ARCHIVED WITH THE LEDGER: the two scenario-authoring helpers that only the
   ledger's "Planning Scenarios" menu and Strategy Library ever called. They
   were lifted out of src/10-app.js at the same time — modelStrategy in
   particular reads STRATEGY_LIBRARY, which is defined above in this file and
   is no longer part of the loaded app, so leaving it in 10-app.js would have
   left a live reference to a symbol that no longer exists.

   They are closures over App state (scenarios, active, status, year,
   deepClone, logEvent, setScenarios); to restore them, paste them back inside
   the App component rather than calling them from here.
   ========================================================================== */
...
/* Strategy Scenario Library "Model scenario": clone the active scenario,
   apply the strategy's real input changes (STRATEGY_LIBRARY, above), and add
   the result to the scenario list. Same deepClone + setScenarios + audit-log
   path as every other scenario — the baseline scenario is never touched. */
/*
